...
static struct sched_domain_attr g_dattr_new[MAX_SCHED_DOMAIN_NUM];
#endif
...
#ifdef CONFIG_HISI_CPUSET_DOUBLE_FREE_FIX
	if (dattr) {
		if (ndoms > MAX_SCHED_DOMAIN_NUM) {
			WARN_ON(1);
			ndoms = MAX_SCHED_DOMAIN_NUM;
		}
		memcpy(g_dattr_new, dattr, ndoms * sizeof(struct sched_domain_attr)); //lint !e670
		kfree(dattr);
		*attributes = g_dattr_new;
	} else {
		*attributes = NULL;
	}
#else
