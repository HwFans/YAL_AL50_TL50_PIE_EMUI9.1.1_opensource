...
void note_task_waking(struct task_struct *p, u64 wallclock)
{
	...
	walt_update_task_avg_sleep(p, wallclock - p->last_sleep_ts);
#endif
#ifdef CONFIG_HISI_ED_TASK
	/*
	 * Note:
	 * These members in ED_TASK will be checked in the following call:
	 * set_task_cpu->walt_fixup_busy_time->migrate_ed_task->is_ed_task
	 * So, make sure note_task_waking is called before set_task_cpu in ttwu.
	 */
	p->last_wake_wait_sum = 0;
	p->last_wake_ts = wallclock;
#endif
...
#ifdef CONFIG_HISI_CPUSET_DOUBLE_FREE_FIX
#define MAX_SCHED_DOMAIN_NUM 30
static struct sched_domain_attr g_dattr_cur[MAX_SCHED_DOMAIN_NUM];
#endif
...
#ifdef CONFIG_HISI_CPUSET_DOUBLE_FREE_FIX
	if (dattr_new) {
		if (ndoms_new > MAX_SCHED_DOMAIN_NUM) {
			WARN_ON(1);
			ndoms_new = MAX_SCHED_DOMAIN_NUM;
		}
		memcpy(g_dattr_cur, dattr_new, ndoms_new * sizeof(struct sched_domain_attr));
		dattr_cur = g_dattr_cur;
	} else {
		dattr_cur = NULL;
	}
	doms_cur = doms_new;
