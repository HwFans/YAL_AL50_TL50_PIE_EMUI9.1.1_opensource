...
int32 free_seperted_rx_buf(uint8 subsys, uint8 alloctype)
{
    ...
    spin_lock(&pst_sepreted_data->sepreted_rx_lock);
    if (pst_sepreted_data->rx_buf_org_ptr != NULL) {
        if (alloctype == KZALLOC) {
...
int32 hw_bfgx_open(uint32 subsys)
{
    ...
    if (BFGX_POWER_SUCCESS != alloc_seperted_rx_buf(subsys, bfgx_rx_max_frame[subsys], VMALLOC)) {
                          ...
                          size_t count, loff_t *f_pos)
{
    ...
    if (0 == skb->len) { /* curr skb data have read to user */
        kfree_skb(skb);
...
STATIC ssize_t hw_bt_write(struct file *filp, const int8 __user *buf, size_t count, loff_t *f_pos)
{
    ...
    if (BT_TYPE_DATA_LEN == bt_data_combination.len) {
                          ...
                          size_t count, loff_t *f_pos)
{
    ...
    if (0 == ps_core_d->bfgx_info[BFGX_FM].rx_queue.qlen) { /* if don,t data, and wait timeout function */
                             ...
                             size_t count, loff_t *f_pos)
{
    ...
    ps_get_core_reference(&ps_core_d);
    if (unlikely((NULL == ps_core_d) || (NULL == buf) || (NULL == filp) ||
        (NULL == ps_core_d->ps_pm) || (NULL == pm_data))) {
        PS_PRINT_ERR("ps_core_d is NULL\n");
        return -EINVAL;
    }
...
STATIC int64 hw_gnss_ioctl(struct file *file, uint32 cmd, uint64 arg)
{
    ...
    if (cmd == PLAT_GNSS_DCXO_SET_PARA_CMD) {
    ...
    if (cmd == PLAT_GNSS_ABB_CLK_PARA_CMD) {
        puser = (int32 __user *)arg;
                             ...
                             size_t count, loff_t *f_pos)
{
    ...
    if (0 == ps_core_d->rx_dbg_seq.qlen) { /* if no data, and wait timeout function */
        if (filp->f_flags & O_NONBLOCK) {  /* if O_NONBLOCK read and return */
            return -EAGAIN;
        }
...
int32 uart_loop_test_send_data(struct ps_core_s *ps_core_d, uint8 *buf, size_t count)
{
    ...
    while (count > 0) {
        ...
        if (count > GNSS_TX_PACKET_LIMIT) {
            if (false == start) { /* this is a start gnss packet */
