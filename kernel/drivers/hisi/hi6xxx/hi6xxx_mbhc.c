...
#include "hisi/hi6555v3_utility.h"
#include "hisi/hi6555v3_pmu_reg_def.h"
#include "hisi/hi6555v3.h"
...
__attribute__ ((weak)) int fm_lan_notifier_call_chain(unsigned long val)
{
	...
	logi("fm_lan_notifier_call_chain: use weak\n");
	return 0;
}
	...
	case IRQ_STAT_PLUG_IN:
		if (check_usb_analog_hs_support()) {
			...
			if (ret == USB_ANA_HS_PLUG_IN) {
				logi("usb ananlog hs is PLUG_IN\n");
			} else {
				logi("usb ananlog hs is PLUG_OUT\n");
				ret = USB_ANA_HS_PLUG_OUT;
			}
		} else {
			ret = irq_state;
		}
...
static void _usb_ana_hs_recheck(struct hi6xxx_mbhc_priv *priv, int hkadc_value)
{
	hi6xxx_irqs_mask_set(priv, IRQ_MSK_COMP);
	hi6xxx_hs_micbias_power(priv, false);
...
static void _hs_plug_in_detect(struct hi6xxx_mbhc_priv *priv)
{
	...
	usb_ana_hs_support = check_usb_analog_hs_support();
	if ((HI6XXX_JACK_BIT_LINEOUT != priv->hs_status) && (HI6XXX_JACK_BIT_NONE != priv->hs_status) && (priv->hs_status != priv->old_hs_status)) {
		if (usb_ana_hs_support &&
			(priv->hs_status == HI6XXX_JACK_BIT_INVERT ||
			priv->hs_status == HI6XXX_JACK_BIT_HEADPHONE))
			_usb_ana_hs_recheck(priv, hkadc_value);
		...
		/* recheck usb ana headset when hs status is invert or headphone and same as old */
		if (priv->usb_ana_need_recheck && usb_ana_hs_support &&
			(priv->hs_status == HI6XXX_JACK_BIT_INVERT || priv->hs_status == HI6XXX_JACK_BIT_HEADPHONE)) {
			_usb_ana_hs_recheck(priv, hkadc_value);
...
static void hi6xxx_mbhc_ana_usb_cfg_get(struct device_node *np,
	struct hi6xxx_mbhc_priv *priv)
{
	unsigned int val = 0;
...
static void hi6xxx_plug_in_detect(void *priv)
{
	struct hi6xxx_mbhc_priv *private_data = (struct hi6xxx_mbhc_priv *)priv;

	_hs_plug_in_detect(private_data);
}

static void hi6xxx_plug_out_detect(void *priv)
{
	struct hi6xxx_mbhc_priv *private_data = (struct hi6xxx_mbhc_priv *)priv;

	_hs_plug_out_detect(private_data);
}

static void hi6xxx_hs_high_resistence_enable(void *priv, bool enable)
{
...
static struct usb_analog_hs_dev usb_analog_dev = {
	...
	.ops = {
		...
		.plug_in_detect = hi6xxx_plug_in_detect,
		.plug_out_detect = hi6xxx_plug_out_detect,
		.get_headset_type = NULL,
		.hs_high_resistence_enable = hi6xxx_hs_high_resistence_enable,
	},
};
...
int hi6xxx_mbhc_init(struct snd_soc_codec *codec, struct hi6xxx_mbhc **mbhc)
{
	...
	hi6xxx_mbhc_ana_usb_cfg_get(np, priv);
	...
	if (ret) loge("Not support usb analog headset\n");
