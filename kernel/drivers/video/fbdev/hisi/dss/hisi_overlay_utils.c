...
void hisi_dss_mctl_on(struct hisi_fb_data_type *hisifd, int mctl_idx, bool enable_cmdlist, bool fastboot_enable)
{
	...
	#if CONFIG_SH_AOD_ENABLE
		if ((mctl_idx == DSS_MCTL0) && get_aod_support()) {
			// clear aod channel[D2 D3 D0 D1] here before display
			set_reg(mctl_base + MCTL_CTL_MUTEX_RCH0, 0x0, 32, 0);
			set_reg(mctl_base + MCTL_CTL_MUTEX_RCH1, 0x0, 32, 0);
			set_reg(mctl_base + MCTL_CTL_MUTEX_RCH6, 0x0, 32, 0);
			set_reg(mctl_base + MCTL_CTL_MUTEX_RCH7, 0x0, 32, 0);
		}
	#endif
