...
static int hisifb_dss_get_platform_limit(struct hisi_fb_data_type *hisifd, void __user *argp)
{
	...
	get_platform_limit.max_hwc_mmbuf_size = MMBUF_SIZE_MAX;
	get_platform_limit.max_mdc_mmbuf_size = MMBUF_SIZE_MDC_MAX;
	...
	HISI_FB_INFO("max_hwc_mmbuf_size=%d, max_mdc_mmbuf_size=%d, fold_display_support=%d!\n",
		...
		get_platform_limit.fold_display_support);
	return 0;
}
...
/*
 * 1. to fix the orlando resume to light the lcd on fpga platform.
 * 2. if not fpga platform,do nothing and return 0.
*/
static int hisi_fb_pm_resume_on_fpga(struct device *dev)
{
	int ret = 0;
	if(NULL == dev)
	{
	...
	if(1 == g_fpga_flag) { //fpga platform
		ret = hisi_fb_pm_resume(dev);
		if (ret != 0) {
		    HISI_FB_ERR("failed to hisi_fb_pm_resume! ret=%d\n", ret);
	    }
	}
	return 0;
}
...
static struct dev_pm_ops hisi_fb_dev_pm_ops = {
	...
	/*to fix the orlando resume to light the lcd on fpga platform.*/
	.resume = hisi_fb_pm_resume_on_fpga,
};
