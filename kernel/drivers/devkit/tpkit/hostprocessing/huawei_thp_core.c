...
static int thp_spi_transfer_one_byte_bootloader(
			struct thp_core_data * const cd,
...
static int thp_open(struct inode *inode, struct file *filp)
{
	 ...
	 */
	__set_bit(THP_STAUTS_WINDOW_UPDATE, (unsigned long *)&cd->status);
	__set_bit(THP_STAUTS_TOUCH_SCENE, (unsigned long *)&cd->status);
...
static long thp_ioctl_spi_sync(void __user *data, unsigned int lock_status)
{
	struct thp_core_data *cd = thp_get_core_data();
...
void thp_spi_cs_set(u32 control)
{
	int rc = 0;
	struct thp_core_data *cd = thp_get_core_data();
	struct thp_timing_config *timing = &cd->thp_dev->timing_config;
