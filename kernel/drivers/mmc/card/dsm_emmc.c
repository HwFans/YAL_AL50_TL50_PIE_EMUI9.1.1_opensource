...
int dsm_emmc_get_log(void *card, int code, char *err_msg)
{
	...
	if (dsm_emmc_process_log(code, err_msg)) {
		/*clear global buffer*/
		memset(g_emmc_dsm_log.emmc_dsm_log, 0, buff_size); /* unsafe_function_ignore: memset */
...
#pragma GCC diagnostic ignored "-Wformat"
				ret = snprintf(dsm_log_buff, buff_size, "manufacturer_id:%04x Error Num:%lu, %s", /* unsafe_function_ignore: snprintf */
...
#pragma GCC diagnostic pop
				...
				ret = snprintf(dsm_log_buff, buff_size, "product_name:%s ", /* unsafe_function_ignore: snprintf */
				...
				ret = snprintf(dsm_log_buff, buff_size, /* unsafe_function_ignore: snprintf */
					"fw:%02x%02x%02x%02x%02x%02x%02x%02x ",
				...
				ret = snprintf(dsm_log_buff, buff_size, "manufacturer_date:%02x\n", (card_dev->raw_cid[3]>>8)&0xff); /* unsafe_function_ignore: snprintf */
				if (ret < 0) {
					pr_err("%s:%d ++\n", __func__, __LINE__);
					ret = 0;
				}
			...
			if (EMMC_EXT_CSD_LENGHT < buff_size) {
				if (NULL != card_dev->cached_ext_csd) {
					...
					for (i = 0; i < EMMC_EXT_CSD_LENGHT; i++) {
						ret = snprintf(dsm_log_buff, buff_size, "%02x", ext_csd[i]); /* unsafe_function_ignore: snprintf */
						if (ret < 0) {
							pr_err("%s:%d ++\n", __func__, __LINE__);
							ret = 0;
						}
