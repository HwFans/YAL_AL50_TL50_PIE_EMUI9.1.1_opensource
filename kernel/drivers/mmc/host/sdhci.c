		   ...
		   unsigned int *actual_clock)
{
	...
	unsigned int real_div = div, clk_mul = 1;
	u16 clk = 0;
...
int sdhci_runtime_resume_host(struct sdhci_host *host)
{
	struct mmc_host *mmc = host->mmc;
	unsigned long flags;
	unsigned int host_flags = host->flags;
...
int sdhci_setup_host(struct sdhci_host *host)
{
	...
	if (!max_current_caps && !IS_ERR(mmc->supply.vmmc)) {
		...
		if (curr > 0) {
			...
			max_current_caps =
				((unsigned int)curr << SDHCI_MAX_CURRENT_330_SHIFT) |
				((unsigned int)curr << SDHCI_MAX_CURRENT_300_SHIFT) |
				((unsigned int)curr << SDHCI_MAX_CURRENT_180_SHIFT);
		}
	}
