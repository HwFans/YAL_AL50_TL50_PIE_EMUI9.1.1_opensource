...
#define GOODIX_BK_CHIP_ID_ADDR_HIGH  0x00
#define GOODIX_BK_CHIP_ID_ADDR_LOW   0x04
...
{
	uint8_t tx[GOODIX_WRITE_CMD_TX_LEN] = { 0 };
	uint32_t tx_len;
	union SPI_CTRL ctrl;
...
static void set_goodix_ud_config(GPIO_NUM_TYPE gpio_reset,
	GPIO_NUM_TYPE gpio_cs)
{
	uint8_t tx[GOODIX_WRITE_CMD_TX_LEN] = { 0 }; /* 3 : tx register length */
	uint32_t tx_len;
	union SPI_CTRL ctrl;
	...
	msleep(GOODIX_SENSOR_SLEEP);
	tx[0] = GOODIX_WRITE_CMD;
	tx[1] = GOODIX_BK_CHIP_ID_ADDR_HIGH;
	tx[2] = GOODIX_BK_CHIP_ID_ADDR_LOW;
	tx_len = GOODIX_WRITE_CMD_TX_LEN;
	mcu_spi_rw(FP_SPI_NUM, ctrl, tx, tx_len, NULL, 0);
}
