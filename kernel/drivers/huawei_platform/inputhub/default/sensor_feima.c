	...
	struct device_attribute *attr, char *buf)
{
	...
	if (g_semtech_sar_detect_aux.detect_flag == 1) {
		...
		semtech_detect_result_aux = sar_sensor_i2c_detect(g_semtech_sar_detect_aux);
	}
...
void send_dc_status_to_sensorhub(uint32_t dc_status)
{
	...
	if (als_data.als_phone_type == VOGUE_PHONE_TYPE ||
		als_data.als_phone_type == TAH_PHONE_TYPE) {
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	ret = abov_str_to_hex(buf, data, ABOV_WRITE_DATA_LENGTH * 2); // data length is 72
	if (ret) {
...
static DEVICE_ATTR(abov_data_write, 0220, NULL,
	attr_cap_prox_abov_data_write);
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	ret = abov_str_to_hex(buf, data, ABOV_CHECKSUM_DATA_LENGTH * 2); // checksum length is 4
	if (ret) {
		hwlog_err("%s: str2hex failed\n", __func__);
		return count;
	}
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%x,%x", &reg, &val) == 2) {
		hwlog_info("%s,reg = 0x%02x, val = 0x%02x\n",
				__func__, *(uint8_t *)&reg, *(uint8_t *)&val);
		buf_temp[0] = reg;
		buf_temp[1] = val;
...
static DEVICE_ATTR(abov_reg_rw, 0660,
	abov_reg_dump_show, abov_reg_dump_store);
