...
static void usb_analog_hs_plugin_work(struct work_struct *work)
{
	...
	if (g_pdata_mos_switch->ldo_need_control && (g_pdata_mos_switch->mos_switch_phy_ldo != NULL)) {
		is_enabled = regulator_is_enabled(g_pdata_mos_switch->mos_switch_phy_ldo);
		hsl_state = hisi_pmic_reg_read(HI6555V3_CODEC_ANA_RW5_REG & HI6555V3_PAGE_VALUE_MASK) & HI6555V3_HPL_PD_MASK;
		if (hsl_state <= 0) {
			hwlog_info("hsl_state %u, usb_ana_ldo %d, no need disable\n", hsl_state, is_enabled);
			wake_unlock(&g_pdata_mos_switch->wake_lock);
			return;
		}
		if ((is_enabled > 0) && regulator_disable(g_pdata_mos_switch->mos_switch_phy_ldo)) {
			hwlog_err("usb_ana_ldo diable fail, hsl_state %u, last ldo state %d\n", hsl_state, is_enabled);
			wake_unlock(&g_pdata_mos_switch->wake_lock);
			return;
		}
		hwlog_info("usb_ana_ldo diable already succ, hsl_state %u, last ldo state %d\n", hsl_state, is_enabled);
	}
	wake_unlock(&g_pdata_mos_switch->wake_lock);
...
static void usb_analog_hs_plugout_work(struct work_struct *work)
{
	...
	if (g_pdata_mos_switch->usb_analog_hs_in == USB_ANA_HS_PLUG_IN) {
		...
		if (g_pdata_mos_switch->gpio_mos_switch > 0) {
			...
			if ((is_enabled > 0) && regulator_disable(g_pdata_mos_switch->mos_switch_phy_ldo)) {
				hwlog_err("error: regulator disable failed, last ldo state %d\n", is_enabled);
				wake_unlock(&g_pdata_mos_switch->wake_lock);
				return;
			}

			hwlog_info("usb_ana_ldo disable already succ, last ldo state %d\n", is_enabled);
			audio_pmic_ldo_set.pmic_num =
				(g_pdata_mos_switch->gpio_usb_lr > 0) ? 0 : 1;
			audio_pmic_ldo_set.pmic_power_type = VOUT_BOOST_EN;
			audio_pmic_ldo_set.pmic_power_voltage = USB_PMIC_VOLTAGE;
