...
static const struct snd_kcontrol_new dapm_soundtrigger_multimic_switch_controls =
	SOC_DAPM_SINGLE("ENABLE", VIRTUAL_UP_REG, SOUNDTRIGGER_MULTI_MIC_EN_BIT, 1, 0);
...
static int play_path_config(struct snd_soc_codec *codec, int event)
{
	int ret = 0;
	...
	case SND_SOC_DAPM_POST_PMD:
		hi64xx_update_bits(codec, SC_CODEC_MUX_CTRL38_REG,
			0x1 << DACSL_MIXER4_DVLD_SEL_OFFSET, 0);
		hi64xx_update_bits(codec, SC_CODEC_MUX_CTRL9_REG,
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	ret = slimbus_param_pass(codec,
		SLIMBUS_6405_TRACK_AUDIO_PLAY, &priv->play_params, event);
	if (ret != 0) {
		AUDIO_LOGW("slimbus fail: %d", ret);
		return ret;
	}
	ret = play_path_config(codec, event);
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	ret = slimbus_param_pass(codec,
		SLIMBUS_6405_TRACK_AUDIO_PLAY, NULL, event);
	if (ret != 0) {
		AUDIO_LOGW("slimbus fail: %d", ret);
		return ret;
	}
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	ret = slimbus_param_pass(codec,
		SLIMBUS_6405_TRACK_DIRECT_PLAY, &priv->play_params, event);
	if (ret != 0) {
		AUDIO_LOGW("slimbus fail: %d", ret);
		return ret;
	}
	ret = play96k_path_config(codec, event);
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	play192k_path_config(codec, event);
...
static int soundtrigger_u5u6_power_event(struct snd_soc_codec *codec,
	slimbus_track_param_t *params, int event)
{
	...
	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
		up_src_pre_pmu(codec, 2, SAMPLE_RATE_16K, SRC_MODE_6);
		u56_select_pga(codec);
		ret = slimbus_track_activate(SLIMBUS_DEVICE_HI6405,
			SLIMBUS_6405_TRACK_SOUND_TRIGGER, params);
		if (ret != 0)
			AUDIO_LOGE("slimbus_track_activate sound trigger err: %d", ret);
		break;
	case SND_SOC_DAPM_POST_PMD:
		/* slimbus soundtrigger deactive */
		ret = slimbus_track_deactivate(SLIMBUS_DEVICE_HI6405,
			SLIMBUS_6405_TRACK_SOUND_TRIGGER, NULL);
		if (ret != 0)
			AUDIO_LOGE("slimbus_track_deactivate sound trigger err: %d", ret);
		u56_select_dspif(codec);
		up_src_post_pmu(codec, 2);
		break;
	default:
		AUDIO_LOGW("power event err: %d", event);
		break;
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	return soundtrigger_u5u6_power_event(codec,
...
static int soundtrigger_4mic_power_event(struct snd_soc_dapm_widget *w,
	struct snd_kcontrol *kcontrol, int event)
{
	int ret = 0;
	struct snd_soc_codec *codec = snd_soc_dapm_to_codec(w->dapm);
	...
	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
		up_src_pre_pmu(codec, 4, SAMPLE_RATE_16K, SRC_MODE_6);
		hi64xx_update_bits(codec, S1_MIXER_EQ_CLK_EN_REG,
			0x1 << RST_5MIC_S3_ACCESS_IRQ_OFFSET,
			0x1 << RST_5MIC_S3_ACCESS_IRQ_OFFSET);
		u56_select_pga(codec);
		ret = slimbus_track_activate(SLIMBUS_DEVICE_HI6405,
			...
			&platform_data->soundtrigger_params);
		if (ret != 0)
			AUDIO_LOGE("slimbus_track_activate sound trigger err: %d", ret);
		hi64xx_update_bits(codec, S1_MIXER_EQ_CLK_EN_REG,
			0x1 << RST_5MIC_S3_ACCESS_IRQ_OFFSET, 0x0);
		break;
	case SND_SOC_DAPM_POST_PMD:
		/* slimbus soundtrigger deactive */
		ret = slimbus_track_deactivate(SLIMBUS_DEVICE_HI6405,
...
#define SWITCH_WIDGET \
	...
	SND_SOC_DAPM_SWITCH_E("SOUNDTRIGGER_MULTIMIC_SWITCH", \
		SND_SOC_NOPM, 0, 0, &dapm_soundtrigger_multimic_switch_controls, \
		soundtrigger_4mic_power_event, \
		(SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD)), \
