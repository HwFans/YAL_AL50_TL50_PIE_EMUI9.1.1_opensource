...
static const struct reg_page hi6405_reg_info[] = {
	...
	{ BASE_ADDR_PAGE_VIRTUAL, VIR_UP, VIR_DOWN },
	...
	unsigned int addr)
{
	...
	if (addr == VIRTUAL_DOWN_REG)
		ret = platform_data->virtual_reg[VIR_UP];
	else if (addr == VIRTUAL_UP_REG)
...
static const struct reg_seq_config pga_fade_regs[] = {
	...
	{ { S2_IL_PGA_CTRL0_REG, 0x1 << S2_IL_PGA_FADE_EN_OFFSET,
...
#ifdef CONFIG_HUAWEI_DSM
	audio_dsm_report_info(AUDIO_CODEC, DSM_HIFI_AK4376_CODEC_PROBE_ERR,
