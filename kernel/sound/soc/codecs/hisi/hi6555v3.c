...
static void hi6555v3_usb_ana_ldo_ctl(struct hi6555v3_priv *priv, bool state)
{
	if (priv == NULL) {
		HI6555V3_LOGE("hi6555v3_priv is NULL\n");
		return;
	}

	if (priv->usb_ana_ldo == NULL) {
		HI6555V3_LOGE("usb_ana_ldo regulators is NULL\n");
		return;
	}

	int is_enable = regulator_is_enabled(priv->usb_ana_ldo);
	if (state && is_enable <= 0) {
		if (regulator_enable(priv->usb_ana_ldo)) {
			HI6555V3_LOGE("couldn't enable usb_ana_ldo regulators\n");
			return;
		}
		HI6555V3_LOGI("enable usb_ana_ldo regulators already succ\n");
	} else if (!state && is_enable > 0) {
		if (regulator_disable(priv->usb_ana_ldo)) {
			HI6555V3_LOGE("couldn't disable usb_ana_ldo\n");
			return;
		}
		HI6555V3_LOGI("disable usb_ana_ldo regulators already succ\n");
	}
}
		...
		struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		...
		hi6555v3_usb_ana_ldo_ctl(priv, true);
		break;
	case SND_SOC_DAPM_PRE_PMD:
		hi6555v3_usb_ana_ldo_ctl(priv, false);
		hi6555v3_hplr_pga_power_off(codec);
		break;
	default:
		HI6555V3_LOGE("power mode event err : %d\n", event);
...
static int _pmu_resource_init(struct snd_soc_codec *codec)
{
	...
	if (!of_property_read_u32(np, "usb_ana_ldo_need_control", &val)) {
		priv->usb_ana_ldo_need_control = val;
	} else {
		priv->usb_ana_ldo_need_control = 0;
	}

	priv->usb_ana_ldo = NULL;
	if (priv->usb_ana_ldo_need_control > 0) {
		priv->usb_ana_ldo = devm_regulator_get(codec->dev, "usb_ana_ldo");
		if (IS_ERR(priv->usb_ana_ldo)) {
			ret = PTR_ERR(priv->usb_ana_ldo);
			HI6555V3_LOGE("couldn't get usb_ana_ldo regulator ret:%ld\n", ret);
			priv->usb_ana_ldo = NULL;
		}
		HI6555V3_LOGI("usb_ana_ldo resource init ok\n");
	}
